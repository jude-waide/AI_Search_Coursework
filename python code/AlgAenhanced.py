...
if __name__ == '__main__':
    ...
    alpha = 1
    beta = 2
    evaporationRate = 0.11
    w = 6
    gamma = 0.3     # How much pheromone is redistrubuted when pheromone smoothing occurs
    stagnationThreshold = 0.8    # Percentage of ants which must have the same tour length to register as stagnation
    num_processes = 6   # Intel i5-8365U is quad-core, 8 threads
    timed = True   # Enable 1 minute timer
  
    

    # FUNCTION USED ONLY BY MAIN PROCESS
    
    # Finds tour using nearest neighbour algorithim
    def nearestNeighbour(dist_matrix):
        tour = [0]
        candidates = [x for x in range(1,num_cities)]
        for i in range(num_cities-1):
            minDist = dist_matrix[candidates[0]][tour[-1]]
            index = 0
            for j in range(1,len(candidates)):
                dist = dist_matrix[candidates[j]][tour[-1]]
                if dist < minDist:
                    minDist = dist
                    index = j
            tour.append(candidates[index])
...
# FUNCTION USED BY ALL PROCESSES

# Adds next city to ant tour
def step(ants, offset, index, candidates, dist_matrix, pheromones, alpha, beta):
    totalProb = 0
    probs = []
    
    # Calculate the probability of each candidate city
    for x in candidates:
        dist = dist_matrix[ants[offset+index]][x]
        if (dist == 0): # Avoid division by 0
            dist = 1
        prob = pheromones[getPheromoneIndex(ants[offset+index], x)]**alpha * (1/dist)**beta
        totalProb += prob
        probs.append(prob)
    
    # Get a random value between 0 and totalProb
    val = random()*totalProb
    i = 0
    prob = probs[i]
    if val < totalProb: # Catch a rare occurance where, if totalProb is near zero, val >= totalProb
        while val >= prob:  # Find the city which val corresponds to
            i += 1
            prob += probs[i]

    ants[offset+index+1] = candidates[i]    # Add city to tour
    del candidates[i]   # Delete chosen city from candidates

# Main loop of worker process
def asyncAntTour(num_cities, alpha, beta, dist_matrix, orders : Queue, results : Queue):
    while True:
        # Waits until an order has been recieved
        offset, numberOfAnts, pheromones = orders.get()

        ants = [0 for x in range(numberOfAnts*num_cities)]
        for antNumber in range(numberOfAnts):
            # Set starting city, every ant starts on a different city
            ants[antNumber*num_cities] = offset + antNumber

            # Create the candidate cities and remove the starting city 
            candidates = [x for x in range(0,num_cities)]
            del candidates[offset+antNumber]

            # Build the rest of the tour, except for the last city
            for i in range(num_cities-2):
                step(ants, antNumber * num_cities, i, candidates, dist_matrix, pheromones, alpha, beta)
            
            # Set last city to be only remaining candidate
            ants[antNumber*num_cities + num_cities - 1] = candidates[0]
        # Return the results
        results.put(ants)

# Gets the index of the edge between city a and b
def getPheromoneIndex(a,b):
    n = max(a,b)
    return n*(n-1)//2 + min(a,b)

if __name__ == "__main__": 
    # Initialisation
    tour = nearestNeighbour(dist_matrix)
    tour_length = cost(tour)
    initialPheromone = (0.5*w*(w-1))/(evaporationRate * tour_length)
    pheromones = [initialPheromone for x in range((num_cities*(num_cities-1))//2)]  # Pheromones stored in a 1D array
    originalityMatrix = [0 for x in range((num_cities*(num_cities-1))//2)]  # Keeps track of how many times each edge has been traversed

    # Multiprocessing
    antsPerProcess = num_ants//num_processes    # Number of ants a process should compute each iteration
    remainingAnts = num_ants%num_processes
    results = Queue()   # Queue for recieving resultings from worker processes
    orders = Queue()    #Queue for giving orders to worker processes

    processes = [Process(target=asyncAntTour, daemon=True, args=(
            num_cities,
            alpha,
            beta,
            dist_matrix,
            orders,
            results,)) for x in range(num_processes)]   # Create worker processes
    ...
    for t in range(max_it):
        print(t)
        offset = 0
        # (005) Order worker processes to make ant tours
        for x in range(num_processes):
            numAntsToProcess = antsPerProcess
            if x == num_processes-1:
                numAntsToProcess += remainingAnts
            
            orders.put((offset, numAntsToProcess, pheromones))
            offset += numAntsToProcess
        ...
        for i in range(len(pheromones)):
            pheromones[i] = (1-evaporationRate)*pheromones[i]
        
        # Find cost of every tour
        tours = []
        for antNumber in range(num_ants):
            newTour = [ants[x] for x in range(antNumber*num_cities, (antNumber+1)*num_cities)]
            tourCost = cost(newTour)
            tours.append((tourCost, newTour))
        # Rank tours by length
        tours.sort(key = lambda pair: pair[0])

        # Update best tour
        if (tours[0][0] < tour_length):
            tour_length = tours[0][0]
            tour = tours[0][1]
        
        # (003) Update originality matrix
        for pair in tours:
            for i in range(num_cities-1):
                # getPheromoneIndex() is used as the originality matrix is structured in the same way as the pheromone matrix
                originalityMatrix[getPheromoneIndex(pair[1][i],pair[1][(i+1)])] += 1
            originalityMatrix[getPheromoneIndex(pair[1][0],pair[1][num_cities-1])] += 1
        # (004) Rank best by originality
        origRankTours = tours[0:w-1]
        origRankTours.sort(key = lambda pair: sum(originalityMatrix[getPheromoneIndex(pair[1][i],pair[1][(i+1)%num_cities])] for i in range(num_cities)), reverse=True)      
        
        # Deposit Ranked Pheromone
        for r in range(0,w-1):
            antTour = origRankTours[r][1]
            antTourCost = origRankTours[r][0]
            for i in range(num_cities-1):
                pheromones[getPheromoneIndex(antTour[i],antTour[i+1])] += (w-r-1)/antTourCost
            pheromones[getPheromoneIndex(antTour[0],antTour[-1])] += (w-r-1)/antTourCost
        
        # Deposit Elitist Pheromone
        for i in range(num_cities-1):
            pheromones[getPheromoneIndex(tour[i],tour[i+1])] += w/tour_length
        pheromones[getPheromoneIndex(tour[0],tour[-1])] += w/tour_length

        # Enhancement found in https://www.mdpi.com/2076-3417/12/21/11219
        ...
        # (001) Check for stagnation (top x% of ants have same tour length)
        pheromoneSmoothing = True
        n = int(num_ants*stagnationThreshold)   # Calculate how far through the list of ants to check
        for i in range(n):
            if tours[i][0] != tours[i+1][0]:    # If the top n tours are not the same, convergence has not occured
                pheromoneSmoothing = False
                break
        
        # (002) Pheromone Smoothing
        # Enhancement found in https://www.mdpi.com/2076-3417/12/21/11219
        # Pérez-Carabaza S, Gálvez A, Iglesias A. Rank-Based Ant System with Originality Reinforcement and Pheromone Smoothing. Applied Sciences. 2022; 12(21):11219. https://doi.org/10.3390/app122111219
        if pheromoneSmoothing:
            maxP = max(pheromones)  # Get max pheromone level
            minP = min(pheromones)  # Get min pheromone level
            # Adjust all pheromone levels
            for i in range(len(pheromones)):
                pheromones[i] = gamma * initialPheromone + ((pheromones[i]-minP)/(maxP-minP))*(1-gamma)*initialPheromone
        ...
        if (timed and (time.time()-start_time) + (iteration_time*1.1) > 59.5):
            break
    added_note += "Finished after " + str(t) + " iterations\nAdditional parameters:\nevaporationRate - " +  str(evaporationRate) + "\nalpa - " + str(alpha) + "\nw - " + str(w) + "\nbeta - " + str(beta) + "\nnum_processes - " + str(num_processes) + "\ngamma - " + str(gamma) + "\nsmoothingThreshold - " + str(stagnationThreshold)
