...
class Ant():
    ...
    def __init__(self, startCity) -> None:
        self.tour = [0 for x in range(num_cities)]
        self.tour[0] = startCity
        self.candidates = [x for x in range(0, num_cities)]
        del self.candidates[startCity]
        self.index = 0
    ...
    def step(self, dist_matrix):
        ...
        for x in self.candidates:
            prob = pheromones[getPheremoneIndex(self.tour[self.index], x)] * ((1/dist_matrix[self.tour[self.index]][x])**beta)
            totalProb += prob
            probs.append(prob)
        ...
        self.index += 1
        self.tour[self.index] = self.candidates[i]
        del self.candidates[i]


ants = [Ant(x) for x in range(num_ants)]
tour = nearestNeighbour(dist_matrix)
tour_length = cost(tour)
initialPheromone = num_ants/tour_length
#print(initialPheromone)
print(tour)
print(tour_length)
exit()
...
for t in range(max_it):
    ...
    for ant in ants:
        ant.index = 0
        ant.candidates = [x for x in range(0,num_cities)]
        del ant.candidates[ant.tour[0]]
    ...
    for i in range(num_cities-1):
        for j in range(i+1,num_cities):
            pheromones[getPheremoneIndex(i,j)] = (1-evaporationRate)*pheromones[getPheremoneIndex(i,j)]

    localBest = -1
    #Deposit Pheromone
    for ant in ants:
        tourCost = cost(ant.tour)
        if (tourCost < localBest or localBest == -1):
            localBest = tourCost
        if (tourCost < tour_length):
                tour_length = tourCost
                tour = [x for x in ant.tour]
        for i in range(num_cities-1):
            pheromones[getPheremoneIndex(ant.tour[i], ant.tour[i+1])] += 1/tourCost
    print(localBest)
