...
# Added Header guard to stop code being ran by new processes 
if __name__ == '__main__':
    ...
    max_repeated_it = 5
    num_processes = 8   # Intel i5-8365U is quad-core, 8 threads
...
# Functions seen by child processes
def step(ants, offset, index, candidates, dist_matrix, pheromones, beta):
    ...
    for x in candidates:
        prob = pheromones[getPheremoneIndex(ants[offset+index], x)] * ((1/dist_matrix[ants[offset+index]][x])**beta)
        totalProb += prob
        probs.append(prob)
    time1 = time.time_ns()
    ...
    ants[offset+index+1] = candidates[i]
    del candidates[i]
    time3 = time.time_ns()
    return time1 - time0
    
def asyncAntTour(num_cities, beta, dist_matrix, orders : Queue, results : Queue):
    while True:
        offset, numberOfAnts, pheromones = orders.get()

        ants = [0 for x in range(numberOfAnts*num_cities)]
        for antNumber in range(len(ants)//num_cities):
            ants[antNumber*num_cities] = offset + antNumber
            total = 0
            candidates = [x for x in range(0,num_cities)]
            del candidates[offset+antNumber]
            for i in range(num_cities-1):
                total += step(ants, antNumber * num_cities, i, candidates, dist_matrix, pheromones, beta)
        results.put(ants)
    
def getPheremoneIndex(a,b):
    n = max(a,b)
...
if __name__ == "__main__": 
    ...
    pheromones = [initialPheromone for x in range((num_cities*(num_cities-1))//2)]
    repeated_its = 0

    # Multiprocessing
    antsPerProcess = num_ants//num_processes
    ...
    processes = [Process(target=asyncAntTour, daemon=True, args=(
            ...
            dist_matrix,
            orders,
            results,)) for x in range(num_processes)]
    ...
    for t in range(max_it):
        ...
        for antNumber in range(num_ants):
            ...
            # Find new best
            if (tourCost < tour_length):
                tour_length = tourCost
                tour = newTour
            tours.append((tourCost, newTour))

        tours.sort(key = lambda pair: pair[0])
        #print(tours[0][0], tours[1][0], tours[2][0], tours[3][0])
        
        # Check repeated iterations
        if (tours[3][0]) == tour_length:
            repeated_its += 1
            if repeated_its >= max_repeated_it:
                break
        
        # Deposit Ranked Pheromone
        for r in range(0,w-1):
            antTour = tours[r][1]
            antTourCost = tours[r][0]
            for i in range(num_cities-1):
                pheromones[getPheremoneIndex(antTour[i],antTour[i+1])] += (w-r-1)/antTourCost
            pheromones[getPheremoneIndex(antTour[0],antTour[-1])] += (w-r-1)/antTourCost
        
        # Deposit Elitist Pheromone
        for i in range(num_cities-1):
            pheromones[getPheremoneIndex(tour[i],tour[i+1])] += w/tour_length
        pheromones[getPheremoneIndex(tour[0],tour[-1])] += w/tour_length
    added_note += "Finished after " + str(t) + " iterations\nAdditional parameters:\nevaporationRate - " +  str(evaporationRate) + "\nw - " + str(w) + "\nbeta - " + str(beta) + "\nmax_repeated_it - " + str(max_repeated_it) + "\nnum_processes - " + str(num_processes)
