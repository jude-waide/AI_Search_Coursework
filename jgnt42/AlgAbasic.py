...
class Ant():
    ...
    def __init__(self) -> None:
        self.tour = [0 for x in range(num_cities)]
        self.candidates = [x for x in range(1, num_cities)]
        self.index = 0
        self.tourCost = 0
    
    # Adds next city to tour
    def step(self, dist_matrix):
        totalProb = 0
        probs = []

        # Calculate the probability of each candidate city
        for x in self.candidates:
            dist = dist_matrix[self.tour[self.index]][x]
            if (dist == 0): # Avoid division by 0
                dist = 1
            prob = pheromones[getPheremoneIndex(self.tour[self.index], x)]**alpha * (1/dist)**beta
            totalProb += prob
            probs.append(prob)

        # Get a random value between 0 and totalProb
        val = random.random()*totalProb
        i = 0
        ...
        self.index += 1
        self.tour[self.index] = self.candidates[i]  # Add city to tour
        del self.candidates[i]  # Delete chosen city from candidates


# INITIALISATION
ants = [Ant() for x in range(num_ants)]
tour = nearestNeighbour(dist_matrix)
tour_length = cost(tour)
initialPheromone = (0.5*w*(w-1))/(evaporationRate * tour_length)
pheromones = [initialPheromone for x in range((num_cities*(num_cities-1))//2)]  # Pheremones stored in 1D array
...
for t in range(max_it):
    ...
    for ant in ants:
        ant.index = 0
        ant.candidates = [x for x in range(1,num_cities)]
    ...
    for ant in ants:
        ant.tourCost = cost(ant.tour)
        if (ant.tourCost < tour_length or tour_length == -1):
            tour_length = ant.tourCost
            tour = [x for x in ant.tour]
    ants.sort(key = lambda ant: ant.tourCost)

    # Deposit Ranked Phereomone
    for r in range(0,w-1):
        ant = ants[r]
        for i in range(num_cities-1):
            pheromones[getPheremoneIndex(ant.tour[i],ant.tour[i+1])] += (w-r-1)/ant.tourCost
        pheromones[getPheremoneIndex(ant.tour[0],ant.tour[-1])] += (w-r-1)/ant.tourCost
