...
class Individual:
    ...
    def crossOver(self, other : 'Individual') -> 'Individual':
        #Swaps two substrings, of the same size and in the same position, between the two parents

        child1 = Individual(len(self.tour))
        child2 = Individual(len(self.tour))

        i = int(random.random()*(len(self.tour)))
        prefix1 = self.tour[0:i]
        prefix2 = other.tour[0:i]
        suffix1 = other.tour[i:len(other.tour)]
        suffix2 = self.tour[i:len(other.tour)]

        # Fix the children to remove duplicates
        duplicates = []
        for i in range(len(suffix1)):
            if (suffix1[i] in prefix1):
                duplicates.append(i)
        for i in range(len(suffix2)):
            if (suffix2[i] in prefix2):
                j = duplicates.pop(0)
                suffix1[j], suffix2[i] = suffix2[i], suffix1[j]
                
        child1.tour = prefix1 + suffix1
        child2.tour = prefix2 + suffix2

        child1.tourLength = cost(child1.tour)
        child2.tourLength = cost(child2.tour)
    ...
    # Exchange mutation - Swaps two random elements
    def mutate(self):
        i1 = int(random.random()*len(self.tour))
        i2 = int(random.random()*len(self.tour))
        self.tour[i1], self.tour[i2] = self.tour[i2], self.tour[i1]
        self.tourLength = cost(self.tour)
    
    # Fitness function - 1 over the tour length
    def calculateFitness(self):
        self.fitness = 1/self.tourLength

# Selects two random parents from population using roulette wheel
def selection(population: list[Individual], totalFitness: float) -> tuple[Individual, Individual]:
    parents = []
    for i in range(2):
...
for individual in population:
    individual.tourLength = cost(individual.tour)
    individual.calculateFitness()

tour = population[0].tour
tour_length = population[0].tourLength
...
for t in range(max_it):
    ...
    for i in range(pop_size):
        parent1, parent2 = selection(population, totalFitness)  # Select parents
        child : Individual = parent1.crossOver(parent2)     # Produce chuld
        ...
        # Update best
        if child.tourLength < tour_length:
            tour = child.tour
            tour_length = child.tourLength
